...
* Namespacing the content type identifiers to allow entitities (including xmtp.org) to manage their respective domain
* Versioning the identifier format to allow future changes to it
* Versioning the content type to allow its future evolution
...
The API currently accepts only `string` as the message content, which suggests to the user that only plain text is supported.
We need to enable at least future possibility of using different types of content beyond just plain text (rich text, images, video, sound, file attachments, etc). However given that this is a large and complex topic we don't want to have to solve it all right now.
We want a flexible framework that will allow building a rich library of various types of supported content over time. This library should be open to collaboration with other organizations. We want to keep the framework simple, but powerful enough to not hinder future development. The framework should also provide a reasonably 
...
We will not introduce a separate version for the embedded type, it can be tied to the version of the outer message type. Change in either of these will trigger version change of the overall protocol.

### Content Type Identifier and Parameters

The `contentType` string identifies the type and format of the information contained in the content. It needs to carry enough information to be able to route the decoding process to the correct decoding machinery. As such the identifier should carry following bits of information:

* authority ID
* content type ID
* content type version (major.minor)

Identifier format version is tied to the protocol version. Changes to the format will require corresponding protocol version adjustments. Such changes may add new information to the identifier but it should only be information that is required to match the identifier with the corresponding decoding machinery or information that is required for all content types (like the content type version). Any information that is specific to a content type should be carried in the content type parameters field.

Authority ID identifies the entity that governs a set of content types, their definitions and implementations. `xmtp.org` is one such organization. Authority ID should be unique and be widely recognized as belonging to the entity. DNS domains or ENS names can serve this purpose (e.g. `uniswap.eth`). The authority is responsible for providing a definition of the content type and its encoding parameters as well as the associated implementation.
...
```ts
export const ContentTypeText = '1/xmtp.org/text/1'
...
export default class Client {
  ...
  constructor(waku: Waku, keys: PrivateKeyBundle) {
    ...
    this._encoders = this.defaultEncoders()
  }

  private defaultEncoders(): Map<string, ContentEncoder<any>> {
    return new Map([[ContentTypeText, new TextContentEncoder()]])
  }
  ...
```
...
export const ContentTypeAlternativeDescription =
  '1/xmtp.org/alternative-description/1'
...
This doesn't comply with XIP-0 but is a more sensible way to show the incremental change of the SDK.
