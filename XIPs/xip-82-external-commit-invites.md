...
sequenceDiagram
    ...
    Note over Members: validate against EXTERNAL_COMMIT_POLICY,<br/>the atomic commit shape,<br/>and max_uses (entries tagged with external_group_id)
    Members-->>Svc: on epoch advance: committer refreshes immediately;<br/>others backstop via existence check (refresh_pointers)
```
...
//     also differ.
//
//   * `allow_external_commit = true` itself authorizes the one write
//     the atomic external-commit shape requires: the joiner inserting
//     its OWN GROUP_MEMBERSHIP entry. There is no separate permissions
//     grant for it — in every state validators accept, such a grant's
//     value would be fully determined by this switch (enabled ⇒ must
//     admit, disabled ⇒ never consulted), and a field whose value is
//     forced is a tripwire, not configuration. Writes to any OTHER
//     component are governed by that component's
//     `external_committer_permissions` block (deny-by-default; the v1
//     atomic shape permits none).
message ExternalCommitPolicyV1 {
  ...
  // commit, in nanoseconds measured from the START OF THAT GROUPINFO'S
  // EPOCH — the delivery-service envelope timestamp of the commit that
  // began the epoch (see "Receive-side validation" check 9) — NOT
  // wall-clock GroupInfo export time, which is not on the wire and
  // cannot be validated convergently. 0 = no staleness limit. An
  // external commit whose envelope timestamp exceeds its referenced
  // epoch's start by more than `expire_in_ns` is rejected. Narrows the
  // replay window for stolen-blob attacks and forces re-export
  // frequency. (The effective-expiry fold in `EncryptedGroupInfoBlobV1`
  // uses this same epoch-start origin.)
...
- The blob the key wraps contains an MLS `GroupInfo` for the *current* epoch only — not historical messages. A compromised key permits joining the group (which `allow_external_commit` already gates), not retroactive decryption.
...
This is the per-component declarative authorization layer for external committers — the symmetric twin of the existing `permissions` field, but evaluated against Sender = `NewMemberCommit` rather than existing members. When an external committer's commit proposes to modify a component's value, the validator consults that component's `external_committer_permissions` block; absent means denied.

One write is deliberately **not** governed by this layer: the joiner inserting its own `GROUP_MEMBERSHIP` entry — the write the atomic external-commit shape *requires* on every join — is authorized by `EXTERNAL_COMMIT_POLICY.allow_external_commit` itself. An explicit grant for it would carry no information: in every state validators accept, its value is forced by the switch (enabled ⇒ it must admit, or every join dead-ends at validation; disabled ⇒ it is never consulted). Deriving it from the switch removes a cross-component coupling that the enabling commit would otherwise have to establish atomically.

In v1 this makes `external_committer_permissions` a reserved surface: the atomic shape forbids an external commit from touching any component other than the joiner's own membership entry, so no block is ever consulted. It exists (deny-by-default) for future flavors — e.g. a resync flavor updating its own entry, or any joiner write beyond the atomic shape — which will be governed declaratively per component rather than by more policy switches.
...
**`GROUP_MEMBERSHIP` entry field (for `max_uses`).** `GROUP_MEMBERSHIP` is a `TlsMap<InboxId, Bytes>` whose per-member value is itself a protobuf message, so recording which invite admitted a member is a routine additive field — not a structural change to the map. This XIP adds an optional `admitted_via_external_group_id` to that per-member message: an external committer sets it to the `external_group_id` carried in the `EXTERNAL_COMMIT_POLICY` of the very `GroupInfo` it commits against — i.e. the slot active at that epoch, *not* the raw QR value — **in the same entry it already writes**, so the atomic shape above is unchanged; it is absent for members added by `Welcome`. The live use-count is the number of entries whose `admitted_via_external_group_id` equals the active `external_group_id`.

This is why requiring `admitted_via == active` (receive-side check 10) cannot break a legitimate in-flight join across an `external_group_id` rotation: rotation is itself an epoch-advancing commit, so a join either sequences *before* it — validating against the pre-rotation state, where the `GroupInfo`'s id is the active id the joiner recorded, so check 10 passes — or *after* it, in which case the joiner's commit was built on the now-superseded epoch and is rejected as stale by MLS before check 10 is reached. The joiner never predicts a future slot id; it records the one bound to the epoch it actually joins (and step 6 of the join flow already aborts when the QR's id disagrees with that `GroupInfo`).
...
7. **Lookup by `(epoch, group_state_hash)` (SHOULD).** Beyond serving the whole set (rule 3), a service SHOULD let a caller fetch only the candidate(s) whose plaintext `(epoch, group_state_hash)` match a supplied pair. This is the member-refresh fast path (see "Keeping the blob fresh"): a member computes its current pair locally and pulls exactly the matching blob instead of the full retention window. This is **retrieval, not arbitration** — it does not order, evict, coalesce, or pick a winner (rules 2 and 5 are unchanged); it only narrows what the caller chooses to download. The pair is a safe lookup key because it is member-derivable yet unforgeable to non-members (see "Keeping the blob fresh").
8. **Existence query (SHOULD).** A service SHOULD expose a presence check — "does the slot hold at least one candidate advertising `(epoch, group_state_hash)`?" — returning a boolean (or a small descriptor: a count, the matching nonces / content hashes) **without** the ciphertext body. This is the cheapest realization of the member backstop's stand-down decision (a member that finds its current pair present skips both the download and the re-upload) and of cross-mirror consistency checks. A `present` result is a **liveness signal, not a validity proof** — the plaintext pair is attacker-copyable (rule 5), so members back it with an occasional full fetch-and-validate (see "Keeping the blob fresh"). A minimal service implementing only rule 3 still interoperates: members fall back to fetch-and-filter, at higher bandwidth.
...
10. The joiner's new entry records `admitted_via_external_group_id` equal to the **active** `external_group_id` — required on every external commit, whatever `max_uses` is (see "Atomic external commit shape").
11. When `EXTERNAL_COMMIT_POLICY.max_uses != 0`: the number of current `GROUP_MEMBERSHIP` entries tagged with the active `external_group_id` is strictly less than `max_uses`. The commit that would create the (`max_uses`+1)-th concurrent invited member is rejected.

The joiner's own-entry insert needs no permissions consultation — check 7 is its authorization (see "Per-component authorization"); checks 4–5 bound it to exactly that entry. Any `AppDataUpdate` beyond the atomic shape is already rejected by check 5 before `external_committer_permissions` would apply.
...
5. Order all survivors by **descending epoch**, and within each epoch by deterministic `group_state_hash` (e.g. ascending). Attempt them in that order; on a commit rejected at validation, exclude that candidate and continue to the next — **falling through to lower-epoch survivors** when every highest-epoch fork is rejected, rather than abandoning a valid older blob. (A deterministic *single* pick is not sufficient — it would re-select the same losing fork on every re-fetch and loop forever; and stopping at the highest epoch would strand a valid lower-epoch blob whenever the freshest candidates are all losing forks.) If all survivors are exhausted, abort and retry later — a newer genuine blob may appear.
...
Every commit advances the epoch — a rename, a `Welcome`-add, a key update — and an external commit built from a stale-epoch `GroupInfo` is rejected by every member. Without a refresh path beyond join-side step 10, a printed QR dies the moment the group has any commit activity and stays dead until the issuing admin happens to come back online. Three roles keep slots alive, in decreasing frequency and increasing cost:

- **The committer — primary, immediate, no download.** The member whose commit merged (a joiner is the committer of its own external commit — join-side step 10 is exactly this case) already holds the `GroupInfo` it just produced and has provably won the epoch. It re-wraps that `GroupInfo` (fresh random nonce, effective `expires_at_ns`) and uploads to every `refresh_pointers` slot **immediately — no jitter**. This is one wrap and one upload by the single best-placed party — online by definition, no fetch, no decrypt — and covers the steady state: after an ordinary commit the slot is current before anyone else looks.

- **Other members — cheap backstop, no download.** Non-committers do **not** react per commit. On a low-frequency trigger (app-foreground, a slow timer, or a reported join failure) a member reconciles each `refresh_pointers` slot:
  1. Compute the group's current `(epoch, group_state_hash)` **locally** — free, from its own `GroupContext`; no fetch.
  2. **Existence-check** the slot for that pair (Service contract rule 8; fall back to a metadata fetch where the service does not expose it).
  3. **Present** → stand down: the pair is member-derivable but unforgeable to non-members (below), so its presence means a member already produced a current blob.
  4. **Absent** → mint a fresh blob from the member's *own* group state (export `GroupInfo`, fresh nonce, effective expiry) and upload. A backstop member never downloads a peer's ciphertext — it has the state to produce its own.

- **Periodic deep check — the self-heal.** The existence check trusts plaintext metadata, and that metadata is attacker-copyable (Service contract rule 5): a keyless writer that has *read* a genuine blob can stamp its `(epoch, group_state_hash)` onto junk ciphertext, so a `present` result proves a genuine blob *was uploaded*, not that a *valid* one is *currently retained* — a flood may have evicted the genuine blob (see "Slot poisoning"). To close that gap, a member SHOULD — far less often than the fast backstop (a slow timer, and on a reported join failure) — perform a **full fetch-and-validate**: download the slot's candidates, validate them as a joiner would (AEAD + consistency), and re-upload if none is valid at the current epoch. This is the only refresh step that reads a peer's ciphertext, and it is rare, so the per-commit cost stays at zero downloads for non-committers.

**Why the existence check is meaningful — non-members cannot forge the pair.** `group_state_hash` is `digest(GroupContext)`, and `GroupContext` (RFC 9420 §8.1) binds `tree_hash` — a hash over the full ratchet tree including the fresh HPKE public keys minted along the committer's update path, distributed only to members — and `confirmed_transcript_hash`, which chains the commit's `confirmation_tag`, a MAC under the `confirmation_key` derived from the group's secret key schedule. An outsider watching the delivery topic — including a stranger holding the public QR — can predict `epoch` (a counter) and perhaps `group_id`, but cannot compute either member-only input, so it cannot produce the pair from scratch (~256-bit hash/MAC security). The pair therefore acts as a *witness that a member acted*: a slot advertising the member's current pair was populated by a member (or copied from one — which still implies a genuine upload). It is not an authenticator of any particular ciphertext, which is why the deep check above backs it.

The scheme is self-selecting and offline-tolerant: the committer almost always wins, a backstop member that finds the witness present spends only a metadata round-trip, and any single online member suffices to repopulate a stale slot. Duplicate uploads from a read-write race are harmless — the versioned slot retains them and the joiner validates-and-selects. A group whose members are all offline is not producing the commits that stale the blob in the first place.

**Net cost of enabling external commits.** Members never download a peer's ciphertext on the common path: the committer has its own `GroupInfo`, a backstop member mints from its own state, and the freshness check is metadata-only. A full candidate download happens only on the join path (a non-member with no group state — see "Join-side flow") and on the rare deep-check self-heal. Per commit, the cost is one upload by the committer; per backstop tick, a cheap existence query — not a multi-megabyte download per member per commit.
...
A `refresh_pointers` entry MAY be an `https_url` (libxmtp performs the existence-check, fetch, and upload directly) or an `opaque` pointer (resolved by the application, exactly as an opaque `service_pointer` is — see `ServicePointer`). For an `opaque` pointer libxmtp cannot interpret the transport, so the refresh steps above run **through the application**: libxmtp supplies the crypto (compute the current `(epoch, group_state_hash)`, wrap a fresh blob, judge candidates) and the application performs the I/O against its own resolver. A group whose members run different applications SHOULD therefore prefer `https_url` refresh pointers — an `opaque` pointer is refreshed only by members whose application understands it; members on other applications skip it (the same liveness cost as omitting it for them).
...
- **Enable atomicity**: When `allow_external_commit` transitions to `true`, the proposal MUST also populate `symmetric_key` (a 32-byte `SymmetricKey`) and `external_group_id` (at least 4 bytes). There is no group state in which the master switch is on but the invite coordinates are absent. Validators reject an enable whose resulting state violates this (the check is on post-state, so it converges). The enable is a single-component write: the joiner-self-entry authorization is the switch itself (see "Per-component authorization"), so no companion `COMPONENT_REGISTRY` write exists to couple with. `refresh_pointers` SHOULD also be populated in the same proposal when the group wants member-driven refresh (see "Keeping the blob fresh").
...
The following test cases define the expected coverage. The cases exercising this revision's features — typed primitives, versioned-slot selection, and `max_uses` accounting (notably 11–16) plus the member-refresh cases (23–25) — land with the review-follow-up PRs noted under "Reference implementation"; a reader checking out the predating PRs will not yet find all of them.
...
9. **External committer attempts to rename the group via `GROUP_NAME`**: rejected by the atomic-shape check (the only AppDataUpdate an external commit may carry is the joiner's own `GROUP_MEMBERSHIP` entry); `GROUP_NAME`'s `external_committer_permissions` (all-Deny by default) would deny it regardless.
...
- **Fetch hygiene for `https_url`.** Clients MUST require the `https` scheme and MUST ignore credentials embedded in the URL. Clients MUST NOT follow redirects to a different scheme, and SHOULD NOT follow redirects to a different origin (host/port) — a same-scheme cross-origin 30x can still bounce the scanner to an attacker-controlled server, leaking its IP and timing and potentially serving attacker content; treat a cross-origin redirect as a fetch failure (the joiner falls through to the next candidate / retries later) unless the application has explicitly allowlisted the target. Scanning a QR and fetching immediately reveals the scanner's IP address (and scan time) to whoever controls the URL — before the user has decided anything. Clients SHOULD obtain user confirmation before the first fetch for an invite, and MAY route fetches through a proxy.
...
"The service learns nothing about the group contents" is true of content, not of traffic. A slot's host observes: ciphertext sizes (which scale with group size — the blob carries the full ratchet tree), upload cadence (join and commit activity), fetch patterns (scan volume, timing, scanner IP addresses and geography), and slot identifiers. With `refresh_pointers` populated, refresh activity also touches the service — not only joiners' and scanners' fetches: the committer of each epoch-advancing commit uploads, and other members issue periodic existence queries (Service contract rule 8) plus a rare full fetch. The existence query reveals to the service which `(epoch, group_state_hash)` a member asks about and when — i.e. the group's current epoch and a member's refresh cadence — though the wrapped state itself stays inside the AEAD. It never learns message content, group metadata values, or member identities — all of that is inside the AEAD. Deployments that care SHOULD pad ciphertexts to size buckets before upload, and MAY front fetches and refreshes through a proxy or CDN so client addresses terminate there; both are application policy, outside libxmtp.
...
| Non-member with payload | Touch components beyond `GROUP_MEMBERSHIP` | Validator rejects: forbidden by the atomic commit shape (`external_committer_permissions` would also deny: all-Deny default) |
